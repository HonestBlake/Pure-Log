...
namespace pureLog{ // #scope: pureLog
    ...
    MODULE_EXPORT using DefaultOutputStream = StdCout<OutputStream::UNBUFFERED>;
    MODULE_EXPORT inline constexpr bool USING_LEVEL_FORMATS = true;
    MODULE_EXPORT inline constexpr bool USING_COLORS = true;

    // #CLASS: Logger<T_Derived>
    MODULE_EXPORT template<class T_Derived, bool t_usingLevelFormatting = USING_LEVEL_FORMATS, bool t_usingColors = USING_COLORS> class Logger{
    public:
    ...
    // Public Methods
        ...
        T_Derived& time(const TimePoint& p_time = std::chrono::system_clock::now());
    ...
    // Protected Methods
        ...
        virtual std::string getFormattedTime(const std::variant<Log::Level, std::string>& p_level, const TimePoint& p_time);
